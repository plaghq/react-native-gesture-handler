...
class PanGestureHandler extends _GestureHandler.default {
  ...
  onPointerUp(event, sourceEvent) {
    super.onPointerUp(event);

    if (sourceEvent !== null && sourceEvent !== void 0 && sourceEvent.cancelable) {
  ...
  onPointerRemove(event, sourceEvent) {
    if (sourceEvent !== null && sourceEvent !== void 0 && sourceEvent.cancelable) {
      sourceEvent.preventDefault();
    }
  ...
  onPointerMove(event, sourceEvent) {
    if (sourceEvent) {
      const state = this.getState();

      if (sourceEvent.cancelable && state === _State.State.ACTIVE) {
        sourceEvent.preventDefault();
      }
  ...
  onPointerOutOfBounds(event, sourceEvent) {
